...
Full-stack microservices application that helps users manage job applications with intelligent resume analysis.
- **AI engine** (Python + FastAPI + spaCy + Scikit-learn) calculates resume-to-job match scores, identifies missing keywords, and generates tailored improvement suggestions.
- **Backend** REST API built with Spring Boot 3 and Java; **frontend** built with React + Vite + Recharts for application analytics dashboards.
- Orchestrated with Docker Compose for single-command startup; CI/CD pipeline via GitHub Actions (lint → test → containerize).

[View on GitHub](https://github.com/rathan-raj/Job-Assistant)
...
A full-stack security monitoring application that detects unauthorized access in real time.
- Modern React/TypeScript frontend with Tailwind CSS for live monitoring dashboards.
- Python backend handles detection logic and alert processing.
